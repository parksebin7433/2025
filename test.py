...
available_tops = sorted(list(set(available_tops)))

selected_top = st.selectbox("👕 상의 색을 골라주세요:", available_tops)

# 선택한 상의 색이 있는 스타일 자동 선택
style_candidates = [stl for stl in outfit_palette[season] if selected_top in outfit_palette[season][stl]]
style = random.choice(style_candidates)  # 후보 중 랜덤 선택
