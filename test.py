...
# 상의 색상 전체 리스트
all_tops = ["빨강","주황","노랑","초록","파랑","보라","핑크","화이트","검정","그레이"]
...
outfit_palette = {
    "봄": {
        "캐주얼": {
            ...
            "그레이": {"하의": ["블랙", "네이비"], "신발": ["화이트", "블랙"], "가방": ["실버", "화이트"]}
        },
        "포멀": {
    ...
    # 여름 / 가을 / 겨울도 같은 방식으로 정의 (일부 색상만 있어도 괜찮음)
    # 팔레트에 없는 상의 색은 default_palette 적용
    "여름": {...}, 
    "가을": {...},
    "겨울": {...}
}

# 기본 팔레트 (팔레트에 없는 색상 선택 시 사용)
default_palette = {"하의": ["블랙", "화이트"], "신발": ["화이트", "블랙"], "가방": ["블랙", "화이트"]}
...
if st.button("✨ 코디 추천받기"):
    palette = outfit_palette[season][style].get(selected_top, default_palette)
