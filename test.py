import streamlit as st
import random

# 상의 색상 전체 리스트
ALL_TOPS = ["빨강","주황","노랑","초록","파랑","보라","핑크","화이트","검정","네이비","그레이","베이지"]

# 퍼스널컬러별 추천 상의 팔레트
personal_palette = {
    "봄웜": ["노랑", "주황", "베이지", "화이트", "핑크"],
    "여름쿨": ["파랑", "핑크", "민트", "화이트", "보라"],
    "가을웜": ["브라운", "주황", "베이지", "카키", "올리브"],
    "겨울쿨": ["블랙", "네이비", "빨강", "화이트", "보라"]
}

# 계절 + 스타일별 색상 팔레트 (상의 → 나머지 아이템)
outfit_palette = {
    "봄": {
        "캐주얼": {
            "화이트": {"하의": ["데님 블루", "베이지"], "신발": ["화이트", "핑크"], "가방": ["브라운", "화이트"]},
            "빨강": {"하의": ["화이트", "블랙"], "신발": ["화이트", "블랙"], "가방": ["네이비", "화이트"]},
            "노랑": {"하의": ["화이트", "네이비"], "신발": ["화이트", "브라운"], "가방": ["브라운", "화이트"]},
        },
        "포멀": {
            "화이트": {"하의": ["블랙", "네이비"], "신발": ["블랙", "실버"], "가방": ["블랙", "네이비"]},
            "검정": {"하의": ["화이트", "그레이"], "신발": ["블랙", "화이트"], "가방": ["실버", "브라운"]},
        },
        "스포츠": {
            "그레이": {"하의": ["블랙", "네이비"], "신발": ["화이트", "민트"], "가방": ["블랙", "그레이"]},
            "화이트": {"하의": ["블랙", "그레이"], "신발": ["화이트", "레드"], "가방": ["블랙", "화이트"]},
        }
    },
    "여름": {
        "캐주얼": {
            "파랑": {"하의": ["화이트", "데님 블루"], "신발": ["화이트", "네이비"], "가방": ["화이트", "블루"]},
            "화이트": {"하의": ["데님 블루", "베이지"], "신발": ["화이트", "민트"], "가방": ["화이트", "네이비"]},
        },
        "포멀": {
            "네이비": {"하의": ["화이트", "그레이"], "신발": ["블랙", "화이트"], "가방": ["브라운", "네이비"]},
        },
        "스포츠": {
            "화이트": {"하의": ["블랙", "네이비"], "신발": ["화이트", "레드"], "가방": ["화이트", "블랙"]},
        }
    },
    "가을": {
        "캐주얼": {
            "브라운": {"하의": ["베이지", "카키"], "신발": ["브라운", "화이트"], "가방": ["베이지", "브라운"]},
            "주황": {"하의": ["베이지", "네이비"], "신발": ["화이트", "브라운"], "가방": ["브라운", "화이트"]},
        },
        "포멀": {
            "베이지": {"하의": ["브라운", "네이비"], "신발": ["브라운", "블랙"], "가방": ["브라운", "블랙"]},
        },
        "스포츠": {
            "카키": {"하의": ["블랙", "브라운"], "신발": ["화이트", "블랙"], "가방": ["브라운", "블랙"]},
        }
    },
    "겨울": {
        "캐주얼": {
            "검정": {"하의": ["화이트", "그레이"], "신발": ["블랙", "화이트"], "가방": ["실버", "블랙"]},
            "빨강": {"하의": ["블랙", "그레이"], "신발": ["화이트", "블랙"], "가방": ["화이트", "네이비"]},
        },
        "포멀": {
            "네이비": {"하의": ["그레이", "블랙"], "신발": ["블랙", "브라운"], "가방": ["네이비", "블랙"]},
        },
        "스포츠": {
            "그레이": {"하의": ["블랙", "네이비"], "신발": ["화이트", "레드"], "가방": ["블랙", "화이트"]},
        }
    }
}

# 스타일별 기본 팔레트
style_defaults = {
    "캐주얼": {"하의": ["데님 블루", "베이지", "화이트"], "신발": ["화이트", "블랙"], "가방": ["브라운", "화이트"]},
    "포멀": {"하의": ["블랙", "네이비", "그레이"], "신발": ["블랙", "브라운", "실버"], "가방": ["블랙", "브라운", "네이비"]},
    "스포츠": {"하의": ["블랙", "그레이", "네이비"], "신발": ["화이트", "블랙", "레드"], "가방": ["블랙", "화이트", "그레이"]},
}

# 색상 코드
color_codes = {
    "화이트": "#FFFFFF", "검정": "#000000", "그레이": "#808080",
    "베이지": "#F5F5DC", "네이비": "#000080", "빨강": "#FF0000",
    "주황": "#FF8C00", "노랑": "#FFD700", "초록": "#228B22",
    "파랑": "#1E90FF", "보라": "#800080", "핑크": "#FFC0CB",
    "브라운": "#8B4513", "카키": "#808000", "데님 블루": "#1E3A5F",
    "실버": "#C0C0C0", "민트": "#98FF98", "레드": "#FF0000"
}

# 색상 박스 출력 함수
def color_box(label, color_name):
    color = color_codes.get(color_name, "#FFFFFF")
    st.markdown(
        f"""
        <div style="display:flex; align-items:center; margin:5px 0;">
            <div style="width:30px; height:30px; background-color:{color};
                        border:1px solid #000; margin-right:10px;"></div>
            <span style="color:#000000; font-weight:bold;">{label}: {color_name}</span>
        </div>
        """,
        unsafe_allow_html=True
    )

# 앱 시작
st.title("👗 사계절 옷 코디 전체 추천 앱")

# 사용자 입력
season = st.selectbox("🍂 계절을 선택해주세요:", list(outfit_palette.keys()))
style = st.selectbox("🎯 스타일을 선택해주세요:", list(outfit_palette[season].keys()))
selected_top = st.selectbox("👕 상의 색을 골라주세요:", ALL_TOPS)

# 퍼스널컬러 선택
personal = st.selectbox("✨ 퍼스널컬러를 선택해주세요:", list(personal_palette.keys()))

# 날씨 조건 선택
weather = st.selectbox("🌤️ 오늘의 날씨는?:", ["맑음", "흐림", "비", "눈", "덥다", "선선하다", "춥다"])

# 아이템 추가 여부
extra_items = st.multiselect("👜 추가 아이템을 선택해주세요:", ["모자", "액세서리", "아우터"])

if st.button("✨ 코디 추천받기"):
    # 퍼스널컬러 가이드 메시지
    if selected_top not in personal_palette[personal]:
        st.warning(f"선택한 상의 색({selected_top})은 {personal}과 잘 맞지 않을 수 있어요. 추천 상의: {', '.join(personal_palette[personal])}")

    # 팔레트 찾기
    palette = outfit_palette[season][style].get(selected_top, style_defaults[style])

    st.subheader(f"{season} · {style} · {personal} 추천 코디 3세트 ✨")
    for i in range(1, 4):
        bottom = random.choice(palette["하의"])
        shoes = random.choice(palette["신발"])
        bag = random.choice(palette["가방"])

        st.markdown(f"### 👗 코디 {i}")
        color_box("👕 상의", selected_top)
        color_box("👖 하의", bottom)
        color_box("👟 신발", shoes)
        color_box("👜 가방", bag)

        # 날씨 조건 반영 메시지
        st.write(f"☁️ 오늘 날씨({weather})에 맞는 코디입니다!")

        # 추가 아이템 출력
        if extra_items:
            st.write("➕ 추가 아이템:", ", ".join(extra_items))

        st.markdown("---")

    st.info("👉 계절·스타일·퍼스널컬러·날씨까지 반영된 오늘의 코디 완성!")
